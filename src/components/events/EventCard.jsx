...
export default function EventCard({ event }) {
  ...
  const nonEnglish = langs.filter((c) => c !== 'en')

  return (
    <div
      onClick={() => navigate(`/events/${event.id}`)}
      className="bg-surface rounded-2xl shadow-sm border border-border cursor-pointer hover:shadow-md transition-shadow overflow-hidden"
    >
      {event.image_url && (
